...
var anm = document.getElementById('but');
var cli = document.getElementById('but2');
var nor = document.getElementById('but3');
var where = document.getElementById('but4');
var sAnimal = document.getElementById('selectMenu');
var animalElement = document.getElementById('animal');
var animalImg = document.getElementById('anmImg');
var animalBName = document.getElementById('zagl');
var animals = [];

// raboti - animalImg.src = "/imgs/animals/animal106.jpg";

socket.on('h1Name', function (zaglavie) {
	animalBName.innerHTML = zaglavie;
});

socket.on('unLockButts', function (success) {
		if(success){
			sAnimal.style.display = 'inline-block';
		}else{
			sAnimal.style.display = 'none';
		}
});

// socket.on('anmName',function (name, ida) {
// 	animals[ida] = ''+name; // ida - raboti
// });
...
anm.onclick = function () {
		...
		if (vsErr) {
			...
			loadTextResource('../shaders/shader.fs.glsl', function (fsErr, fsText) {
				if (fsErr) {
					...
					loadJSONResource('../jsons/earth1.json', function (modelErr, modelObj) {
						if (modelErr) {
							alert('Fatal error getting Susan model (see console)');
							console.error(fsErr);
						} else {

							loadImage('/imgs/Richness_map_of_impacted_mammals.jpg', function (imgErr, img) {
								if (imgErr) {
	...
	socket.emit('isItClicked',true);
	// socket.emit('readAnimalsByC',{
	// 	country:
	// });
				...
				if (fsErr) {
					...
					loadJSONResource('../jsons/earth1.json', function (modelErr, modelObj) {
						if (modelErr) {
							...
							loadImage('/imgs/earth2.jpg', function (imgErr, img) {
								if (imgErr) {
									alert('Fatal error getting Susan texture (see console)');
									console.error(imgErr);
								} else {
				...
				if (fsErr) {
					...
					loadJSONResource('../jsons/earth1.json', function (modelErr, modelObj) {
						if (modelErr) {
							...
							loadImage('/imgs/earth6m.jpg', function (imgErr, img) {
								if (imgErr) {
									alert('Fatal error getting Susan texture (see console)');
									console.error(imgErr);
								} else {
				...
				if (fsErr) {
					...
					loadJSONResource('../jsons/earth1.json', function (modelErr, modelObj) {
						if (modelErr) {
							...
							loadImage('/imgs/earth1.jpg', function (imgErr, img) {
								if (imgErr) {
									alert('Fatal error getting Susan texture (see console)');
									console.error(imgErr);
								} else {
