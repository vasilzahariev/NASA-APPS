...
function populateAnimals(animalElementId) {
    // given the id of the <select> tag as function argument, it inserts <option> tags
    var animalElement = document.getElementById(animalElementId);
    animalElement.length = 0;
    animalElement.options[0] = new Option('Select Animal', '-1');
    animalElement.selectedIndex = 0;
    //for (var i = 0; i < animalO.length; i++) {
    socket.on('anmName',function (name, ida) {
      animalO[ida] = ''+name;
        animalElement.options[animalElement.length] = new Option(name, ida+1);
      });
  //  }
}

function changeAnm() {
    var selectBox = document.getElementById("animal");
    var selectedValue = selectBox.options[selectBox.selectedIndex].value;
    //alert(selectBox.options[selectBox.selectedIndex]);
    socket.emit('infoAnmName',selectedValue);
   }
